...
skeleton.header.sectionEnd.menu.on.click.settings = {
	component: 'button',
	on: {
		click: {
			component: 'section',
...
skeleton.header.sectionEnd.menu.on.click.settings.on.click.language = {
	component: 'select',
	text: 'language',
	options: [{
		value: "en",
	...
	on: {
		change: function () {
			var self = this,
				language = satus.storage.get('language');
			...
			satus.locale.import(language, function () {
				var layers = self.baseProvider.layers[0];

				skeleton.main.layers.rendered.dispatchEvent(new CustomEvent('open'));

				satus.empty(layers.firstChild);

				satus.render(satus.last(layers.path), layers.firstChild);
			}, '_locales/');
		}
	}
};
...
skeleton.header.sectionEnd.menu.on.click.settings.on.click.dateAndTime = {
	component: 'button',
	text: 'dateAndTime',
	before: {
		svg: {
	...
	on: {
		click: {
			...
			use_24_hour_format: {
				...
				value: new Date().toString().match(/am|pm/i) ? false : true
			}
		}
	}
};
...
skeleton.header.sectionEnd.menu.on.click.settings.on.click.backupAndReset = {
	component: 'button',
	text: 'backupAndReset',
	before: {
		svg: {
...
skeleton.header.sectionEnd.menu.on.click.settings.on.click.about = {
	component: 'button',
	text: 'about',
	before: {
		svg: {
