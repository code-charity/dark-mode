...
function getFilters(settings) {
    var string = '',
        ...
        body_filter = '',
        body_child_filter = '',
        not = ':not([dark]):not([data-color-mode="dark"])';
    ...
    if (settings.bluelight > 0) {
        ...
        if (settings.invert_colors === true || settings.invert_colors === undefined) {
            string += 'html' + not + ',html' + not + ' body{background:#000 0 0 !important}';
    ...
    if (settings.invert_colors === true || settings.invert_colors === undefined) {
        string += 'html' + not + ' body [style*="url("],html' + not + ' body [style*=background-position],html' + not + ' body canvas,html' + not + ' body iframe,html' + not + ' body img:not([src*="/ic_"]):not([src*=_ic_]):not([class*=icon]),html' + not + ' body pre,html' + not + ' body video, html' + not + ' body i{-webkit-filter:invert(1)!important;filter:invert(1)!important}';

        if (!(settings.bluelight > 0) && (settings.invert_colors === true || settings.invert_colors === undefined)) {
            string += 'html' + not + ', htmlbody{background:#fff 0 0 !important}';
    ...
    if (html_filter.length > 0) {
        string += 'html' + not + '{-webkit-filter:' + html_filter + ';filter:' + html_filter + '}html' + not + ' #night-mode-extension-brightness{filter:invert(1)}';
    }

    if (body_filter.length > 0) {
        string += 'html' + not + ' body{-webkit-filter:' + body_filter + ';filter:' + body_filter + '}html' + not + ' #night-mode-extension-brightness{filter:invert(1)}';
    }

    if (body_child_filter.length > 0) {
        string += 'html' + not + ' body > *:not(#night-mode-extension-brightness){-webkit-filter:' + body_child_filter + ';filter:' + body_child_filter + '}';
    }
